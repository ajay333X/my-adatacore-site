...
async function load(){
  ...
  try{
    ...
    $('pageSub').textContent=`${doc.project?.name||'Transcription'} · ${doc.stats?.task_count||0} submitted task${Number(doc.stats?.task_count||0)===1?'':'s'} · link expires ${fmtDate(doc.bundle?.expires_at)}`;
    $('taskCount').textContent=Number(doc.stats?.task_count||0).toLocaleString();
    $('totalDuration').textContent=fmtDuration(doc.stats?.total_duration_ms);
    $('totalWords').textContent=Number(doc.stats?.total_word_count||0).toLocaleString();
    $('notice').textContent='Play any audio directly here. Open a task for the synchronized transcript, timestamp notes, and clarity rating.';
    const rows=Array.isArray(doc.items)?doc.items:[];
    $('taskList').innerHTML=rows.length?rows.map((x,i)=>`<article class="card task-card"><div class="task-top"><div><div class="task-title">${i+1}. ${esc(x.display_name||'Audio')}</div><div class="task-meta">${esc(x.task_id||'Task')} · ${esc(x.source_project_title||'Upload')} / ${esc(x.source_folder||'Root')} · submitted ${esc(fmtDate(x.submitted_at))}</div><div class="task-stats"><span class="pill pill-gray">${esc(fmtDuration(x.duration_ms))}</span><span class="pill pill-gray">${Number(x.segment_count||0)} segments</span><span class="pill pill-gray">${Number(x.word_count||0).toLocaleString()} words</span></div></div><span class="pill pill-brand">${esc(String(x.status||'submitted').replaceAll('_',' '))}</span></div><div class="player-row"><audio controls preload="metadata" src="${esc(x.audio_url||'')}"></audio><a class="btn btn-primary" href="/client-transcript-review.html?bundle=${encodeURIComponent(token)}&item=${encodeURIComponent(x.item_id)}">Open transcript & notes →</a></div></article>`).join(''):'<div class="card empty">No tasks are available in this collection.</div>';
  }catch(e){
