...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const {data:links,error:linksErr}=await supabase.from('transcription_client_bundle_items').select('item_id,position').eq('bundle_id',bundle.id).order('position',{ascending:true});
    if(linksErr)throw linksErr;
    const ids=(links||[]).map((x:any)=>x.item_id);
    if(!ids.length)return json({error:'This review collection is empty.'},404);
    const [{data:project},{data:items,error:itemsErr}]=await Promise.all([
